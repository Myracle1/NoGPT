...
data_human = pd.read_csv(file_path_human, sep="\t", header=None, names=['D_LL', 'Score', 'Perplexity',   'col8', 'col9', 'label'])

# 读取AI数据
file_path_ai ="./AI_data_cn.txt"
data_ai = pd.read_csv(file_path_ai, sep="\t", header=None, names=['D_LL', 'Score', 'Perplexity',   'col8', 'col9', 'label'])
...
# 特征和标签
features = train_data[['D_LL', 'Score' ]]
...
# 保存模型
joblib.dump(svm_model, 'svm_model_cn.pkl')

# 加载模型
svm_model_loaded = joblib.load('svm_model_cn.pkl')

# 做预测
test_features = test_data[['D_LL', 'Score' ]]
