import openai
openai.api_key = "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"
...
n_positions = 512  # 假设的模型最大长度，根据实际情况调整

class PPL_LL_based_gpt3_t5:
    def __init__(self, device="cuda", model_id="gpt2",if_openai=True,openai_model="gpt-3.5-turbo-instruct"):
        self.device = device
    ...
    def perturb_texts_(self,texts, span_length, pct, ceil_pct=False):
            ...
            print(f' {perturbed_texts} texts in {attempts} attempts.')    
            return perturbed_texts

    def get_ll(self,text):
        if self.openai_model!=None:
            print("openai model is used!!!!")        
            kwargs = { "engine": self.openai_model, "temperature": 0, "max_tokens": 50, "logprobs": 1}
    ...
    def calc_ll_result(self,text,num_perturb=10):
        ...
        x=self.perturb_texts_(text_for_perturbed_ll, self.span_length, self.pct)[0]
        return self.get_ll(text),self.get_ll(x)
    ...
    def __call__(self, sentence):
        ...
        # 使用更广泛的正则表达式来匹配所有字母、数字和中文字符
        #total_valid_char = re.findall(r"[\w\u4e00-\u9fa5]+", sentence)
        ...
        # 使用正则表达式按照中文标点符号拆分句子 
        #lines = re.split(f'(?<={chinese_punctuation})\\s*', sentence)
        ...
        #print(f"ll:{ll} perturbed_ll: {perturbed_ll}")

        results["LL"] = ll
        results["Perturbed LL"] = perturbed_ll
        results["D_LL"] = ll-perturbed_ll

        print(f"ll: {ll} average_perturbed_ll: {results['Perturbed LL']} D_ll: {results['D_LL']}")
        ...
        #values = [" LL: ",ll," Perturbed ll: ",results["Perturbed LL"]," D_ll: ",results["D_LL"]," Perplexity: " ,ppl," Perplexity per line: ",fff]
        return results

    def getPPL(self,sentence):
        #if self.openai_model!=None:
            #print("openai model is used in getPPL!!!!")
            #response = openai.Completion.create(
            #model="gpt-3.5-turbo-instruct",
            #prompt=sentence,
            #max_tokens=50,  # 设置生成的最大token数
            #logprobs=2  # 请求返回每个token的对数概率
            # )
            #logprobs = response.choices[0].logprobs.token_logprobs
            #perplexity = -sum(logprobs) / len(logprobs)  # 计算平均perplexity
            #return 0
