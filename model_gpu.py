...
class PPL_LL_based_gpt2_t5:
    ...
    def __call__(self, sentence,mask_model_id,n_perturbations):
        """
        ...
        if mask_model_id == "t5-small":
            print("Using t5-small model & default perturbations=30")
        ...
        for i, line in enumerate(lines):
            ...
            '''
            if len(line) < 20:
                ll_p,p_ll_p=self.calc_ll_result(line)
                p_ll_p=np.array(p_ll_p)
                mean_p_ll_p=np.mean(p_ll_p)
                d_ll_p=ll_p-mean_p_ll_p
                std_p_ll_p=np.std(p_ll_p)
                score_p=d_ll_p/std_p_ll_p
                dll_per_line.append(d_ll_p)
                sll_per_line.append(score_p)
